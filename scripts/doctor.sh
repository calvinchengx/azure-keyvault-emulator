...
#
# Separate from status.sh on purpose: status.sh answers "is the pair usable?"
...
printf '\npython (only needed by `make chain`)\n'
PY="${PY:-}"
if [ -z "$PY" ]; then
...
if [ -n "$PY" ]; then
  ...
  warn "python" 'none of python3/python/py runs; only "make chain" needs it'
fi
...
# predict "the bind will fail" — Rancher Desktop's `steve` holds 127.0.0.1:9443
# and still loses the forward to a container. What is worth knowing is the
# ambiguity: if a probe answers, confirm it is the emulator replying.
printf '\nhost ports this stack binds\n'
if [ -n "$PY" ]; then
...
PORTS = [
    ...
    (9443, "fabric-emulator (--profile full)"),
]
busy = []
for port, who in PORTS:
    ...
    if taken:
        busy.append(port)
        print("  warn  %-28s port %d already answering" % (who, port))
    else:
        print("  ok    %-28s port %d free" % (who, port))
if busy:
    print("        something already listens on %s — often another emulator in" % ", ".join(map(str, busy)))
    print("        this family. Verify with `make status`, not a raw probe.")
EOF
else
