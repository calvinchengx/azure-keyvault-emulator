...
A row is graded 🟢 only when real cryptography or a real Azure SDK does the work — but
...
def green_claims():
    ...
    for line in PARITY.read_text().splitlines():
        ...
        if len(cells) < 3:
            continue
        if cells[0] in ("Key Vault feature", "Capability", "Feature") or set(cells[0]) <= set("-"):
            continue
        if "🟢" in cells[-1]:
