...
func TestValidateHappyPathUserAndApp(t *testing.T) {
	...
	base := mintOpts{iss: testIssuer, aud: VaultAudiences[0], exp: 2000, kid: "test-key"}
	...
	// The trailing-slash audience variant is also accepted, incl. aud-as-array.
	slash := base
	slash.aud = []string{VaultAudiences[1]}
	slash.oid = "oid-2"
	if _, err := v.Validate(mint(t, key, slash)); err != nil {
		t.Fatalf("trailing-slash audience rejected: %v", err)
	}
}

func TestValidateRejections(t *testing.T) {
	v, key := newFixture(t, 1000)
	good := mintOpts{iss: testIssuer, aud: VaultAudiences[0], exp: 2000, oid: "o", kid: "test-key"}
...
func TestExpiryFollowsEmulatorClock(t *testing.T) {
	...
	tok := mint(t, key, mintOpts{iss: testIssuer, aud: VaultAudiences[0], exp: 2000, oid: "o", kid: "k"})
...
func TestValidateRequest(t *testing.T) {
	...
	tok := mint(t, key, mintOpts{iss: testIssuer, aud: VaultAudiences[0], exp: 2000, oid: "o", kid: "test-key"})
