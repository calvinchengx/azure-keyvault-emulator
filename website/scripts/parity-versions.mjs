...
const BASE = '/azure-keyvault-emulator/';
...
export function writeParityHistory(OUT, parity, helpers) {
  ...
  for (const pt of points) {
    ...
    const banner = pt.reconstructed
      ? ''
      : `:::note[Historical snapshot]\nThe feature-parity map as of release **${pt.label}**. The current map is on the [Parity page](/azure-keyvault-emulator/${liveSlug}/).\n:::\n\n`;
  ...
  const clBody =
    `How the [feature-parity map](/azure-keyvault-emulator/${liveSlug}/) changed across releases — ` +
  ...
  const rows = [
    `- **[${version}](/azure-keyvault-emulator/${liveSlug}/)** — the live map on \`main\``,
    ...releasedPoints
      .slice()
      .reverse()
      .map(
        (p) =>
          `- [${p.label}](/azure-keyvault-emulator/parity-history/${versionSlug(p.label)}/) — ` +
          (p.reconstructed ? 'written retrospectively (predates the map)' : 'snapshot at release'),
      ),
  ];
  const idxBody =
    `Versions of the [feature-parity map](/azure-keyvault-emulator/${liveSlug}/), tracked by git release tags. ` +
    `See the [parity changelog](/azure-keyvault-emulator/parity-history/changelog/) for what changed between them.\n\n` +
    rows.join('\n') +
    '\n\n' +
